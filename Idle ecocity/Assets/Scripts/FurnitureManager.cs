...
public class FurnitureManager : MonoBehaviour
{
	...
	public void Load()
	{
		...
		upgradeCostFurniture = double.Parse(PlayerPrefs.GetString("upgradeCostFurniture", "20"));
		upgradeLevelFurniture = PlayerPrefs.GetInt("upgradeLevelFurniture", 0);
		coinsPerSec = double.Parse(PlayerPrefs.GetString("coinsPerSec", "0"));

		//If a furniture has at least an upgrade don't show anymore "clickToEarn" button
		if(upgradeLevelFurniture >= 1)
        {
			idle.clickToEarn.gameObject.SetActive(false);
			idle.autoplay.gameObject.SetActive(true);
        }
	}
	...
	IEnumerator Save()
	{
		...
		//Set the current value of upgrade cost and level of a furniture and coins/second
		PlayerPrefs.SetString("upgradeCostFurniture", upgradeCostFurniture.ToString());
		PlayerPrefs.SetInt("upgradeLevelFurniture", upgradeLevelFurniture);
